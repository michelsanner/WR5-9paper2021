...
def getPDBlines(atoms):
    buf = stringBuffer()
    writePDBStream(buf, atoms)
...
class clusterADCP:
    ...
    def __call__(self, **kw):
        ...
        solutionFile = open('%s_out.pdb'%kw['input'][:-4], 'w')
        if rmsd:
            if hasRef:
                for atom in refAtoms:
	    ...
	    print 'FUGU', len(clusters)
            for i, cl in enumerate(clusters):
                ...
                if i < 10 and backbone:
                    ...
                    prot, NoClash = buildSC(i,allrotamers[cl[0]],outputname=kw['input'][:-4])
                    solutionFile.write("MODEL     %4d\n"%(i+1,))
                    solutionFile.write("USER: ADCP SOLUTION %d\n"%(i+1,))
                    solutionFile.write("USER: SCORE %7.3f\n"%( extE[cl[0]] * 0.59219))
                    lines = getPDBlines(prot.select("not deleted and not hydrogen"))
            ...
            for i, cl in enumerate(clusters):
                NoClash = True
                if backbone and i<300:
                    #prody.writePDB(str(i)+"_bbb.pdb",models._ag,cl[0])
                    writePDB("tmp.pdb",models._ag,cl[0])
                    prot, currpairs, NoClash = buildSC(i,allrotamers[cl[0]],receptor,outputname=kw['input'][:-4])
